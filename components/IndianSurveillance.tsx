...
type SurveillanceTab = 'intake' | 'outbreak' | 'privacy';
      ...
      <div className="flex items-center gap-1.5 bg-slate-100 dark:bg-slate-800 p-1.5 rounded-2xl overflow-x-auto scrollbar-hide">
        {tabs.map(tab => (
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl font-black uppercase tracking-widest text-[10px] transition-all whitespace-nowrap flex-shrink-0 ${activeTab === tab.key
                ...
                <div className="flex flex-wrap gap-1.5">
                  {chain.nodes.map((node, j) => (
                    <React.Fragment key={node.id}>
                      <span className={`px-2 py-1 rounded-lg text-[9px] font-bold ${node.type === 'health_condition' ? 'bg-rose-100 dark:bg-rose-900/40 text-rose-600 dark:text-rose-300' :
...
5. Resource Requirements`;
  ...
  return (
    <div className="space-y-6">
      {predStats && (
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={s.label} className={`border rounded-xl p-4 text-center ${s.danger ? 'bg-rose-50 dark:bg-rose-950/30 border-rose-200 dark:border-rose-800' : 'bg-white dark:bg-slate-800 border-slate-100 dark:border-slate-700'
              }`}>
              ...
              {surveillance.syndromes.map(s => (
                <div key={s.syndromeId} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-xl border border-slate-100 dark:border-slate-600">
                  ...
                  <div className="flex items-center gap-1">
                    <TrendingUp className={`w-3 h-3 ${s.trend === 'rising' ? 'text-rose-500' : s.trend === 'declining' ? 'text-emerald-500 rotate-180' : 'text-slate-400'
                      }`} />
      ...
      {aiOutbreak && (
        <div className={`border rounded-2xl p-5 ${aiOutbreak.risk_level === 'outbreak' ? 'bg-rose-50 dark:bg-rose-950/30 border-rose-200 dark:border-rose-800' :
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 rounded-2xl p-5">
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
            {alerts.slice(0, 20).map(a => (
              <div key={a.id} className={`p-3 rounded-xl border ${a.status === 'outbreak' ? 'bg-rose-50 dark:bg-rose-950/30 border-rose-200 dark:border-rose-800' : 'bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800'
                }`}>
                <div className="flex items-start justify-between mb-1">
                  <div className="flex items-center gap-2">
              ...
              <div key={item.key} className="flex items-center justify-between p-3 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
                ...
                <div className="flex gap-1.5">
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase ${dashboard.consentSettings[item.key] === 'granted'
                  ...
                  <button
                    ...
                    className={`px-2.5 py-1 rounded-lg text-[9px] font-black uppercase ${dashboard.consentSettings[item.key] === 'denied'
          ...
          <div className="space-y-1.5 max-h-48 overflow-y-auto">
            {auditLog.map(entry => (
              <div key={entry.id} className="flex items-center gap-2 p-2 bg-slate-50 dark:bg-slate-700/50 rounded-lg text-[10px]">
                <span className={`w-2 h-2 rounded-full flex-shrink-0 ${entry.processedAt === 'on_device' ? 'bg-emerald-400' : 'bg-blue-400'
                  }`} />
