          ...
          <div className="mb-6 p-5 bg-slate-50 dark:bg-slate-700/50 rounded-2xl border border-slate-200 dark:border-slate-600 space-y-3">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => {
                    setMlApiUrlInput('https://web-production-24253.up.railway.app');
                    // auto-save for convenience
                    setMlApiUrl('https://web-production-24253.up.railway.app');
                    localStorage.setItem('floodMlApiUrl', 'https://web-production-24253.up.railway.app');
                  }}
                  className="px-3 py-2 bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-violet-100 dark:hover:bg-violet-900/40 transition-all border border-slate-300 dark:border-slate-500"
                >
                  Cloud
                </button>
                <button
                  onClick={() => {
                    setMlApiUrlInput('http://localhost:8000');
                    setMlApiUrl('http://localhost:8000');
                    localStorage.setItem('floodMlApiUrl', 'http://localhost:8000');
                  }}
                  className="px-3 py-2 bg-slate-200 dark:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-emerald-100 dark:hover:bg-emerald-900/40 transition-all border border-slate-300 dark:border-slate-500"
                >
                  WSL
                </button>
                <button
                  onClick={saveMlApiUrl}
                  className="px-5 py-2.5 bg-blue-600 text-white rounded-xl font-black text-xs uppercase tracking-widest hover:bg-blue-500 transition-all shadow-md"
                >
                  Save
                </button>
              </div>
            ...
            <p className="text-[10px] font-bold text-slate-400 leading-relaxed">
              Using <span className="text-violet-500 dark:text-violet-400">Cloud Engine</span> for production forecasting, or 
              <span className="text-emerald-500 dark:text-emerald-400"> Local Engine (WSL)</span> for specialized ward analysis.
            </p>
          </div>
        )}
