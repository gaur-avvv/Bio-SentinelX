...
const AppInner: React.FC = () => {
  ...
  const [bioSentinelApiUrl, setBioSentinelApiUrlState] = useState<string>(
    () => localStorage.getItem('biosentinel_ml_api_base_url') || import.meta.env.VITE_BIOSENTINEL_API || 'https://web-production-f898c8.up.railway.app'
  );
  const [surveillanceApiUrl, setSurveillanceApiUrl] = useState<string>(
    () => localStorage.getItem('biosentinel_surveillance_api_base_url') || import.meta.env.VITE_API_BASE_URL || 'https://web-production-37f41.up.railway.app'
  );
  const [floodMlApiUrl, setFloodMlApiUrl] = useState<string>(
    () => localStorage.getItem('floodMlApiUrl') || 'https://web-production-24253.up.railway.app'
  );
  ...
  useEffect(() => {
    localStorage.setItem('floodMlApiUrl', floodMlApiUrl);
  }, [floodMlApiUrl]);
  useEffect(() => {
          ...
          <div className={`max-w-6xl mx-auto w-full ${view === 'assistant' ? 'flex flex-col flex-1 min-h-0' : ''}`}>
            ...
            {view === 'dashboard' ? (
              ...
              <SettingsPage
                ...
                floodMlApiUrl={floodMlApiUrl}
                setFloodMlApiUrl={setFloodMlApiUrl}
                databaseSettings={databaseSettings}
